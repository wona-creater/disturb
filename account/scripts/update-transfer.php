...
if (isset($_POST)) {
      ...
      if (empty($refNumber) || empty($amount) || empty($dated) || empty($description) || empty($type) || empty($scope) || !in_array($status, ['0', '1', '2'])) {
            echo "<script>    
            toastr.error('All fields are required!', 'Empty field', {\"progressBar\": true});
        </script>";
            die();
            ...
            echo "
            <script>
                Swal.fire('Details Updated!', 'Transaction details have been updated', 'success');
                var delayInMilliseconds = 2000;
                setTimeout(function() {
                    window.location.reload();
                }, delayInMilliseconds);
